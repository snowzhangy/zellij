...
final class SettingsStore: ObservableObject {
    ...
    init() {
        ...
        let directory = baseDirectory.appendingPathComponent("Zelmux", isDirectory: true)
        let legacyDirectory = baseDirectory.appendingPathComponent("ZellijAgent", isDirectory: true)
        try? FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        self.url = directory.appendingPathComponent("settings-v1.json")
        let legacyURL = legacyDirectory.appendingPathComponent("settings-v1.json")
    ...
    func updateTouchMode(_ touchMode: TouchMode, for profileID: UUID) {
        guard let index = settings.profiles.firstIndex(where: { $0.id == profileID }) else { return }
        settings.profiles[index].touchMode = touchMode
        saveImmediately()
    }

    func updateLastTabPosition(_ tabPosition: Int, for profileID: UUID) {
        guard let index = settings.profiles.firstIndex(where: { $0.id == profileID }) else { return }
        settings.profiles[index].lastTabPosition = tabPosition
        saveImmediately()
    }
    ...
    func addSnippet(_ snippet: String) {
        ...
        guard !trimmed.isEmpty else { return }
        settings.snippets.removeAll { $0 == trimmed }
        settings.snippets.insert(trimmed, at: 0)
        saveImmediately()
    }

    func updateSnippet(oldValue: String, newValue: String) {
        let trimmed = newValue.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return }
        settings.snippets.removeAll { $0 == oldValue || $0 == trimmed }
        settings.snippets.insert(trimmed, at: 0)
        saveImmediately()
    }

    func deleteSnippet(_ snippet: String) {
        settings.snippets.removeAll { $0 == snippet }
        saveImmediately()
    }
